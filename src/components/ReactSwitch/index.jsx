// src: https://github.com/markusenglund/react-switch/tree/master/src
...
class ReactSwitch extends Component {
  constructor(props) {
    ...
    this.$checkedPos = handleWidth
      ? width - this.$handleWidth - (height - this.$handleDiameter) / 2
      : Math.max(width - height, width - (height + this.$handleDiameter) / 2);
  ...
  $onInputChange(event) {
    ...
    if (Date.now() - this.$lastDragAt > 50) {
      ...
      if (this.$isMounted && Date.now() - this.$lastKeyUpAt > 50) {
  ...
  render() {
    ...
    const checkedIconStyle = {
      ...
      width:
        handleWidth ||
        Math.min(height * 1.5, width - (this.$handleWidth + height) / 2 + 1),
      position: "relative",
      opacity:
